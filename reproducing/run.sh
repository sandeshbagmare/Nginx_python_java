...
#   client -> NGINX(:4088) -> Tomcat + AIAssistantProxyServlet(:4003) -> Uvicorn(:4000)
#
...
start_tomcat() { # $1 = true|false
  MSYS_NO_PATHCONV=1 java -DstripHopByHop="$1" -Dport=4003 -cp "$CP" EmbeddedProxy >/dev/null 2>&1 & TC=$!
  wait_port 4003 /sse
}
...
"$PY" -m uvicorn app:app --app-dir . --host 127.0.0.1 --port 4000 --log-level warning & UV=$!
wait_port 4000 /plain || { echo "uvicorn failed"; exit 1; }
...
echo "  servlet direct :4003   Transfer-Encoding count = $(count_te http://127.0.0.1:4003/sse)   (expect 2)"
S1=$(http_status http://127.0.0.1:4088/sse)
echo "  via NGINX :4088        HTTP $S1                          (expect 502)"
...
echo "  servlet direct :4003   Transfer-Encoding count = $(count_te http://127.0.0.1:4003/sse)   (expect 1)"
S2=$(http_status http://127.0.0.1:4088/sse)
echo "  via NGINX :4088        HTTP $S2                          (expect 200)"
echo "  stream via NGINX:"
curl -s -N --max-time 3 http://127.0.0.1:4088/sse | sed 's/^/      /' | head -6
