...
#   client -> NGINX(:8088) -> Java proxy(:8080) -> Uvicorn(:8000)
#
...
"$PY" -m uvicorn app:app --app-dir python-app --host 127.0.0.1 --port 8000 --log-level warning & UV=$!
wait_port 8000 /plain || { echo "uvicorn failed to start"; exit 1; }
U=$(count_te http://127.0.0.1:8000/sse)
echo "    [1] Uvicorn direct           Transfer-Encoding count = $U   (expect 1)"

echo "==> starting NAIVE proxy :8080"
java java-proxy/NaiveProxy.java & PROXY=$!
wait_port 8080 /sse || { echo "naive proxy failed to start"; exit 1; }
N=$(count_te http://127.0.0.1:8080/sse)
...
wait_port 8088 / >/dev/null 2>&1 || true; sleep 1
S=$(status http://127.0.0.1:8088/sse)
echo "    [3] NGINX -> NAIVE           HTTP status = $S          (expect 502)"
echo -n "        log: "; grep -a 'duplicate header line' "$ERR" | tail -1 | sed 's/^[0-9/ :]*//'

echo "==> swapping NAIVE -> FIXED proxy on :8080"
kill "$PROXY" 2>/dev/null; PROXY=""; sleep 1
java java-proxy/FixedProxy.java & PROXY=$!
wait_port 8080 /sse || { echo "fixed proxy failed to start"; exit 1; }
S2=$(status http://127.0.0.1:8088/sse)
